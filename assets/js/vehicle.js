(function () {
  ...
  function renderVehicle() {
    ...
    var imageSource = window.ANTRAC.imageSource || function (image) { return imageRoot + image; };
    ...
    host.innerHTML =
      '<section class="vehicle-detail"><div class="vehicle-detail__hero">' +
        '<div class="vehicle-detail__main-image"><img src="' + imageSource(vehicle.image) + '" alt="' + vehicle.name + '"></div>' +
      ...
      '<section class="vehicle-gallery-section"><div class="container"><div class="section-heading"><p class="eyebrow">Vehicle gallery</p><h2>Explore the details.</h2></div><div class="vehicle-gallery">' + vehicle.gallery.map(function (image, index) { return '<figure><img loading="lazy" src="' + imageSource(image) + '" alt="' + vehicle.name + ' — image ' + (index + 1) + '"></figure>'; }).join("") + '</div></div></section>';
    host.setAttribute("aria-busy", "false");
  }
